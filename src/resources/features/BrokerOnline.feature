Feature: Broker Online - Legal Business Plan Capturing Process.
  ...
  Scenario Outline: Access Broker Online
    Given User open Broker Online web browser, capture access code '<AccessCode>'  and clicks confirm button.
   ...
   And User should be able to select Product from the drop list '<ClienteleLegalBusinessPlan>' and click on Continue Button to navigate to Business Details.
...
#    And User enter PostalAC '<0152>'
    Examples:
      |AccessCode| code | Password |ClienteleLegalBusinessPlan|CompanyName|
      |0132| b1234  | 12345 |Clientèle Legal Business Plan|Moroni|